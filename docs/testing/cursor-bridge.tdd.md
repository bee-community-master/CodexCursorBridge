...
| 12 | Domain/application imports point inward, source imports are acyclic, implementation files stay below 700 lines, and test suites stay below 900 lines. | `tests/architecture.test.ts` | Architecture | PASS |

## Final verification

- `pnpm verify`: PASS — lint, typecheck, 170 tests, coverage, build, MCP smoke.
- Coverage: statements 87.20%, branches 81.42%, functions 97.19%, lines 90.25% for the core policy modules.
