import { evaluateChangedPaths } from "../domain/repository-path.js";
...
export function assessChanges(input: ChangeAssessmentInput): ChangeAssessment {
  ...
  if (paths.forbidden.length > 0) {
    reasons.push(`Forbidden paths changed: ${paths.forbidden.join(", ")}`);
  }
  if (paths.outOfScope.length > 0) {
    reasons.push(`Out-of-scope paths changed: ${paths.outOfScope.join(", ")}`);
  }
